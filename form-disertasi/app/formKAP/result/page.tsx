...
function QuestionSummaryView({ responses }: { responses: Form2Response[] }) {
  ...
  return (
    <div className="space-y-6">
      {/* Section 1: Komite Audit ↔ Auditor Eksternal */}
      ...
      {/* Section 2: Auditor Eksternal ↔ Auditor Internal */}
      <div className="rounded-2xl border border-orange-200 bg-white/80 shadow-sm backdrop-blur p-6">
        <h3 className="text-xl font-semibold text-orange-900 mb-4 pb-2 border-b border-orange-200">
          Interaksi Auditor Eksternal dengan Auditor Internal
        </h3>
        <div className="space-y-6">
...
export default function Form2ResultsPage() {
  const [responses, setResponses] = useState<Form2Response[]>([]);
  ...
  async function fetchResponses() {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch('/api/form2/responses');
  ...
  function exportIndividualResponses() {
    ...
    // Download file
    const fileName = `form2_individual_responses_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  function exportQuestionSummary() {
    ...
    // Add Section 1: Komite Audit ↔ Auditor Eksternal
    sections.forEach((section) => {
      section.rows.forEach((row) => {
    ...
    // Add Section 2: Auditor Eksternal ↔ Auditor Internal
    sections2.forEach((section) => {
      section.rows.forEach((row) => {
        const total = Object.values(stats[row.id] || {}).reduce((sum, count) => sum + count, 0);
        worksheetData.push({
    ...
    // Download file
    const fileName = `form2_question_summary_${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(wb, fileName);
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 relative overflow-x-hidden">
        ...
        <div className="mx-auto max-w-7xl flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-800 max-w-2xl">
              View all submitted responses from Form 2 (Auditor Eksternal Survey)
            </p>
          </div>
      ...
      <footer className="px-4 sm:px-6 lg:px-8 pb-10">
        <div className="mx-auto max-w-7xl text-xs text-slate-700 text-center">
          © {new Date().getFullYear()} • Form 2 Results - DiluarPersegi
        </div>
      </footer>
    </div>
  );
