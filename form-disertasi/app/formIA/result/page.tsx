...
function QuestionSummaryView({ responses }: { responses: Form3Response[] }) {
    ...
    return (
        <div className="space-y-6">
            {/* Section 1: Auditor Internal dengan Komite Audit */}
            <div className="rounded-2xl border border-blue-200 bg-white/80 shadow-sm backdrop-blur p-6">
                <h3 className="text-xl font-semibold text-blue-900 mb-4 pb-2 border-b border-blue-200">
                    Interaksi Auditor Internal dengan Komite Audit
                </h3>
                <div className="space-y-6">
            ...
            {/* Section 2: Auditor Internal dengan Auditor Eksternal */}
            <div className="rounded-2xl border border-orange-200 bg-white/80 shadow-sm backdrop-blur p-6">
                <h3 className="text-xl font-semibold text-orange-900 mb-4 pb-2 border-b border-orange-200">
                    Interaksi Auditor Internal dengan Auditor Eksternal
                </h3>
                <div className="space-y-6">
...
export default function Form3ResultsPage() {
    const [responses, setResponses] = useState<Form3Response[]>([]);
    ...
    async function fetchResponses() {
        try {
            setLoading(true);
            setError(null);
            const response = await fetch('/api/form3/responses');
    ...
    function exportIndividualResponses() {
        ...
        // Download file
        const fileName = `form3_individual_responses_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    function exportQuestionSummary() {
        ...
        // Add Section 1: Auditor Internal dengan Komite Audit
        sections.forEach((section) => {
            section.rows.forEach((row) => {
                ...
                worksheetData.push({
                    ...
                    'Section': `Internal-Komite - ${section.id}. ${section.title}`,
        ...
        // Add Section 2: Auditor Internal dengan Auditor Eksternal
        sections2.forEach((section) => {
            section.rows.forEach((row) => {
                const total = Object.values(stats[row.id] || {}).reduce((sum, count) => sum + count, 0);
                worksheetData.push({
                    ...
                    'Question ID': row.id,
                    'Question Text': row.label,
                    'Section': `Internal-Eksternal - ${section.id}. ${section.title}`,
                    'SS': stats[row.id]?.['SS'] || 0,
                    'S': stats[row.id]?.['S'] || 0,
                    'AS': stats[row.id]?.['AS'] || 0,
                    'ATS': stats[row.id]?.['ATS'] || 0,
        ...
        // Download file
        const fileName = `form3_question_summary_${new Date().toISOString().split('T')[0]}.xlsx`;
        XLSX.writeFile(wb, fileName);
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 relative overflow-x-hidden">
                ...
                <div className="mx-auto max-w-7xl flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-2 text-slate-800 max-w-2xl">
                            View all submitted responses from Form 3 (Auditor Internal Survey)
                        </p>
                    </div>
            ...
            <footer className="px-4 sm:px-6 lg:px-8 pb-10">
                <div className="mx-auto max-w-7xl text-xs text-slate-700 text-center">
                    © {new Date().getFullYear()} • Form 3 Results - DiluarPersegi
                </div>
            </footer>
        </div>
    );
